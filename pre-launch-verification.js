...
async function testKeysModeSanity() {
  try {
    ...
    logTest(
      ...
      "PASSED",
      `Mode: ${mode} (Key ID: ${redactSecret(TEST_CONFIG.razorpayKeyId, "key_id")})`,
    );
    ...
    logTest(
      "Order Creation Test",
      "PASSED",
      `Order ID: ${testOrder.id}, Auto-capture: ${testOrder.payment_capture ? "Enabled" : "Disabled"}`,
    );
    ...
    try {
      ...
      logTest(
        "Test Order Cleanup",
        "WARNING",
        `Could not cancel test order: ${cancelError.message}`,
      );
    }
    ...
    if (!testOrder.payment_capture) {
      logTest(
        ...
        "WARNING",
        "Auto-capture disabled - uncaptured payments may auto-refund",
      );
    } else {
      logTest(
        "Payment Capture Policy",
        "PASSED",
        "Auto-capture enabled - payments will be captured automatically",
      );
    }
  } catch (error) {
  ...
  try {
    ...
    try {
      ...
      logTest(
        "CORS Allowed Origin",
        "FAILED",
        `Failed for allowed origin: ${error.message}`,
      );
    }
    ...
    try {
      ...
      logTest(
        "CORS Blocked Origin",
        "WARNING",
        "Malicious origin was not blocked",
      );
    } catch (error) {
      if (error.response && error.response.status === 403) {
        logTest(
          "CORS Blocked Origin",
          "PASSED",
          "Malicious origin correctly blocked",
        );
      } else {
        logTest(
          "CORS Blocked Origin",
    ...
    logTest(
      "CORS Configuration",
      "PASSED",
      `Allowed origins: ${corsConfig.allowedOrigins.join(", ")}`,
    );
  } catch (error) {
  ...
  try {
    ...
    if (healthResponse.data.status !== "healthy") {
      logTest(
        "Database Health",
        "FAILED",
        `Database not healthy: ${healthResponse.data.message}`,
      );
      return;
    }
    ...
    logTest(
      "Database Health",
      "PASSED",
      `Status: ${healthResponse.data.status}, Database: ${healthResponse.data.database}, Host: ${healthResponse.data.host}`,
    );
    ...
    try {
      ...
      for (const indexConfig of criticalIndexes) {
        try {
          ...
          if (indexExists) {
            indexResults.push(
              `✅ ${indexConfig.collection}.${Object.keys(indexConfig.index)[0]}`,
            );
          } else {
            ...
            indexResults.push(
              `✅ ${indexConfig.collection}.${Object.keys(indexConfig.index)[0]} (created)`,
            );
          }
        } catch (indexError) {
          indexResults.push(
            `❌ ${indexConfig.collection}.${Object.keys(indexConfig.index)[0]} (${indexError.message})`,
          );
        }
      }
...
async function testPaymentsE2E() {
  try {
    ...
    logTest(
      ...
      "PASSED",
      `Order ID: ${razorpayOrder.id}, Amount: ₹${TEST_CONFIG.testAmount}`,
    );
    ...
    logTest(
      ...
      "PASSED",
      `Generated signature for order: ${razorpayOrder.id}`,
    );

    // Verify key_secret is not exposed client-side
    const configResponse = await axios.get(
      `${API_BASE}/payments/razorpay/config`,
    );
    ...
    if (configResponse.data.keySecret) {
      logTest(
        ...
        "FAILED",
        "Key secret exposed to client",
      );
    } else {
      logTest(
        "Client-Side Secret Exposure",
        "PASSED",
        "Key secret not exposed to client",
      );
    }
    ...
    try {
      ...
      logTest(
        "Test Order Cleanup",
        "WARNING",
        `Could not cancel E2E test order: ${cancelError.message}`,
      );
    }
    ...
    logTest(
      "Payments E2E Test",
      "PASSED",
      `Order ID: ${razorpayOrder.id}, Payment simulation completed`,
    );
  } catch (error) {
...
async function testWebhooksCorrectness() {
  try {
    ...
    logTest(
      "Webhook Idempotency",
      "PASSED",
      `Idempotency test prepared for event: ${testEventId}`,
    );
  ...
  try {
    ...
    try {
      ...
      logTest(
        "Partial Refund",
        "WARNING",
        `Refund test failed (expected for test payments): ${refundError.message}`,
      );
    }
    ...
    try {
      ...
      verificationResults.reconciliationData.successfulPayments =
        successfulPayments;

      logTest(
        "Payment Reconciliation",
        "PASSED",
        `Today's successful payments: ${successfulPayments}`,
      );
    } catch (reconciliationError) {
      logTest(
        "Payment Reconciliation",
        "WARNING",
        `Reconciliation test failed: ${reconciliationError.message}`,
      );
    }
    ...
    logTest(
      "Refunds and Reconciliation Test",
      "PASSED",
      "Refund and reconciliation tests completed",
    );
  } catch (error) {
...
async function testLoggingAndHeaders() {
  try {
    ...
    logTest(
      "Structured Logging",
      "PASSED",
      `Sample logs: ${JSON.stringify(logSamples, null, 2)}`,
    );
    ...
    if (missingHeaders.length > 0) {
      ...
      logTest(
        "Security Headers",
        "PASSED",
        `All security headers present: ${presentHeaders.join(", ")}`,
      );
    }
    ...
    if (csp) {
      logTest(
        "Content Security Policy",
        "PASSED",
        `CSP configured: ${csp.substring(0, 100)}...`,
      );
    } else {
    ...
    logTest(
      "Logging and Headers Test",
      "PASSED",
      "Logging and security headers verification completed",
    );
  } catch (error) {
...
function generateFinalReport() {
  ...
  console.log(
    `📈 Success Rate: ${Math.round((verificationResults.passed / (verificationResults.passed + verificationResults.failed)) * 100)}%`,
  );

  console.log(`\n🔑 Test IDs:`);
  console.log(
    `Order IDs: ${verificationResults.orderIds.join(", ") || "None"}`,
  );
  console.log(
    `Payment IDs: ${verificationResults.paymentIds.join(", ") || "None"}`,
  );
  console.log(
    `Refund IDs: ${verificationResults.refundIds.join(", ") || "None"}`,
  );

  console.log(`\n💰 Reconciliation Data:`);
  console.log(
    `Successful Payments: ${verificationResults.reconciliationData.successfulPayments}`,
  );
  console.log(
    `Internal Paid Orders: ${verificationResults.reconciliationData.internalPaidOrders}`,
  );
  console.log(
    `Mismatches: ${verificationResults.reconciliationData.mismatches.length}`,
  );
  ...
  if (verificationResults.failed === 0) {
    console.log(
      "🟢 GO - All critical tests passed. Ready for production launch.",
    );
  } else {
    console.log(
      "🔴 NO-GO - Critical tests failed. Address issues before launch.",
    );
    console.log("\n🚫 Blockers:");
