...
class SmokeTestSuite {
  ...
  async testAuthenticationFlow() {
    ...
    const profileResult = await this.makeRequest('GET', '/api/auth/profile', null, {
  ...
  async testProductCatalog() {
    ...
    // Test product search
    const searchResult = await this.makeRequest('GET', '/api/products?search=test');
    ...
    const filterResult = await this.makeRequest('GET', '/api/products?category=test');
    ...
    if (productsResult.data.products && productsResult.data.products.length > 0) {
      ...
      const productDetailResult = await this.makeRequest('GET', `/api/products/${productId}`);
  ...
  async testShoppingCart() {
    ...
    // Test add to cart
    const addToCartResult = await this.makeRequest('POST', '/api/cart/add', {
    ...
    if (getCartResult.data.items && getCartResult.data.items.length > 0) {
      ...
      const updateCartResult = await this.makeRequest('PUT', `/api/cart/update/${itemId}`, {
        quantity: 2
      }, {
        'Authorization': `Bearer ${this.authToken}`
      });
  ...
  async testCheckoutFlow() {
    ...
    const createOrderResult = await this.makeRequest('POST', '/api/orders', {
      items: [{
        productId: this.productId,
        quantity: 1,
        price: 100
    ...
    // Test get order
    const getOrderResult = await this.makeRequest('GET', `/api/orders/${this.orderId}`, null, {
      'Authorization': `Bearer ${this.authToken}`
    });
  ...
  async testOrderHistory() {
    ...
    const filteredOrdersResult = await this.makeRequest('GET', '/api/orders?status=pending', null, {
      'Authorization': `Bearer ${this.authToken}`
    });
  ...
  async testPaymentGatewayConfiguration() {
    ...
    const razorpayConfigResult = await this.makeRequest('GET', '/api/payments/razorpay/config');
    ...
    const stripeConfigResult = await this.makeRequest('GET', '/api/payments/stripe/config');
  ...
  async testSecurityHeaders() {
    ...
    const response = await axios.get(`${SMOKE_TEST_CONFIG.baseURL}/api/health`, {
      timeout: 10000
    });
  ...
  async testErrorHandling() {
    ...
    const unauthorizedResult = await this.makeRequest('GET', '/api/auth/profile');
    ...
    const invalidDataResult = await this.makeRequest('POST', '/api/auth/login', {
      email: 'invalid-email',
      password: ''
    });
