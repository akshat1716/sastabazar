...
class WebhookTestSuite {
  ...
  async testWebhookSignatureValidation() {
    ...
    const validSignature = this.generateWebhookSignature(
      payloadString,
      WEBHOOK_TEST_CONFIG.webhookSecret,
    );
    const validResult = await this.sendWebhook(
      "payment.captured",
      payloadString,
      validSignature,
    );
    ...
    const invalidResult = await this.sendWebhook(
      "payment.captured",
      payloadString,
      invalidSignature,
    );
    ...
    const missingSignatureResult = await this.sendWebhook(
      "payment.captured",
      payloadString,
      "",
    );
    ...
    if (
      missingSignatureResult.success ||
      missingSignatureResult.status !== 400
    ) {
  ...
  async testIdempotency() {
    ...
    const firstResult = await this.sendWebhook(
      "payment.captured",
      payloadString,
      signature,
    );
    const secondResult = await this.sendWebhook(
      "payment.captured",
      payloadString,
      signature,
    );
  ...
  async testPortAndProtocol() {
    ...
    // Test HTTPS (port 443)
    const httpsResult = await this.sendWebhook(
      "payment.captured",
      payloadString,
      signature,
    );
    ...
    let httpResult;
    try {
      const httpUrl = WEBHOOK_TEST_CONFIG.baseURL.replace(
        "https://",
        "http://",
      );
      const response = await axios.post(
  ...
  async testErrorHandling() {
    ...
    const malformedSignature = this.generateWebhookSignature(
      malformedPayload,
      WEBHOOK_TEST_CONFIG.webhookSecret,
    );

    const malformedResult = await this.sendWebhook(
      "payment.captured",
      malformedPayload,
      malformedSignature,
    );
    ...
    const emptyPayload = "{}";
    const emptySignature = this.generateWebhookSignature(
      emptyPayload,
      WEBHOOK_TEST_CONFIG.webhookSecret,
    );

    const emptyResult = await this.sendWebhook(
      "payment.captured",
      emptyPayload,
      emptySignature,
    );
