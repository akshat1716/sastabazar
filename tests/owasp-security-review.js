...
class OWASPSecurityReview {
  ...
  async testSecurityMisconfiguration() {
    ...
    if (config.isProduction) {
      ...
      if (debugResponse.success && debugResponse.data) {
        ...
        if (
          responseStr.includes("debug") ||
          responseStr.includes("development") ||
  ...
  generateReport() {
    const criticalCount = this.vulnerabilities.filter(
      (v) => v.severity === "critical",
    ).length;
    const highCount = this.vulnerabilities.filter(
      (v) => v.severity === "high",
    ).length;
    const mediumCount = this.vulnerabilities.filter(
      (v) => v.severity === "medium",
    ).length;
    const lowCount = this.vulnerabilities.filter(
      (v) => v.severity === "low",
    ).length;
  ...
  async runSecurityReview() {
    ...
    try {
      ...
      logger.info(
        ...
        "🔒 OWASP Security Review Complete",
      );
...
if (require.main === module) {
  ...
  securityReview
    ...
    .then((report) => {
      ...
      process.exit(
        report.summary.critical > 0 || report.summary.high > 0 ? 1 : 0,
      );
    })
    .catch((error) => {
