...
class LoadTester {
  ...
  async testEndpoint(endpoint, options = {}) {
    ...
    try {
      ...
      const testResult = {
        ...
        successRate: ((result.requests.total - result.errors - result.timeouts - result.non2xx) / result.requests.total) * 100
      };
      ...
      logger.error({ error, endpoint }, `Load test failed for ${endpoint.name}`);
      throw error;
    }
  }
  ...
  validateResults(result, endpoint) {
    ...
    if (result.latency.average > endpoint.maxLatency) {
      issues.push(`Average latency ${result.latency.average}ms exceeds limit ${endpoint.maxLatency}ms`);
    }

    // Check success rate
    if (result.successRate < 95) {
      issues.push(`Success rate ${result.successRate.toFixed(2)}% below 95% threshold`);
    }
  ...
  async runCriticalEndpointsTest() {
    ...
    return {
      summary: {
        ...
        passedEndpoints: results.filter(r => r.meetsExpectations !== false).length,
        failedEndpoints: results.filter(r => r.meetsExpectations === false).length,
  ...
  async runStressTest(endpoint, maxConnections = 100) {
    ...
    for (const connections of connectionSteps) {
      ...
      try {
        ...
        if (result.successRate < 90 || result.latency.average > endpoint.maxLatency * 2) {
          logger.warn({
            connections,
        ...
        logger.error({ error, connections }, `Stress test failed at ${connections} connections`);
        break;
      }
    }
  ...
  async runDatabasePerformanceTest() {
    ...
    for (const endpoint of dbEndpoints) {
      try {
        ...
        logger.error({ error, endpoint }, `Database performance test failed for ${endpoint.name}`);
      }
    }
  ...
  generateReport() {
    const report = {
      ...
      summary: {
        ...
        passedTests: this.results.filter(r => r.meetsExpectations !== false).length,
        failedTests: this.results.filter(r => r.meetsExpectations === false).length,
  ...
  calculateOverallSuccessRate() {
    ...
    const totalErrors = this.results.reduce((sum, r) => sum + (r.errors || 0) + (r.timeouts || 0) + (r.non2xx || 0), 0);
  ...
  generateRecommendations() {
    ...
    const highLatencyTests = this.results.filter(r => r.latency?.average > 1000);
    ...
    const lowThroughputTests = this.results.filter(r => r.throughput?.average < 100);
  ...
  async runComprehensiveTest() {
    ...
    try {
      ...
      logger.info({ comprehensiveReport: report }, '📊 Comprehensive Load Test Report');
...
if (require.main === module) {
  ...
  loadTester.runComprehensiveTest()
    .then(report => {
      ...
      console.log(`Average Throughput: ${report.summary.averageThroughput.toFixed(2)} req/sec`);
