      ...
      <Link to={`/products/${transformedProduct._id}`} className="block">
        <div className="flex items-center space-x-6 p-4 border-b border-gray-100 hover:bg-gray-50 transition-colors">
          <div className="relative w-24 h-24 flex-shrink-0">
            <img
              src={transformedProduct.images[currentImageIndex]?.url || 'https://via.placeholder.com/96x96?text=No+Image'}
              alt={transformedProduct.images[currentImageIndex]?.alt || transformedProduct.name}
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-sm font-medium text-gray-900 truncate">{transformedProduct.name}</h3>
            <p className="text-xs text-gray-500 mt-1">{transformedProduct.brand}</p>
            <div className="mt-2">
              {transformedProduct.isOnSale ? (
                <div className="flex items-center space-x-2">
                  <span className="text-sm font-medium text-gray-900">{formatCurrency(transformedProduct.salePrice)}</span>
                  <span className="text-xs text-gray-500 line-through">{formatCurrency(transformedProduct.basePrice)}</span>
                </div>
              ) : (
                <span className="text-sm font-medium text-gray-900">{formatCurrency(transformedProduct.basePrice)}</span>
              )}
            </div>
          </div>
          <button
    ...
    <Link to={`/products/${transformedProduct._id}`} className="block group">
      ...
      >
        ...
        <div className="relative aspect-square bg-gray-100 overflow-hidden">
          ...
          {/* Main Image */}
          <img
            src={transformedProduct.images[currentImageIndex]?.url || 'https://via.placeholder.com/400x400?text=No+Image'}
            alt={transformedProduct.images[currentImageIndex]?.alt || transformedProduct.name}
        ...
        <div className="mt-3 space-y-1">
          ...
          <p className="text-xs text-gray-500 uppercase tracking-wide">{transformedProduct.brand}</p>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-1">
              ...
              <span className="text-xs text-gray-500">({transformedProduct.rating.count})</span>
            </div>
          </div>
        </div>
      </motion.div>
