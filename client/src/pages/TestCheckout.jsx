  ...
  return (
    <div className="max-w-2xl mx-auto p-6 text-center">
      ...
      <p className="text-lg text-aura-700 mb-4">
        This page is for testing the checkout flow without needing to log in or
        deal with real payments.
      </p>

      <div className="bg-aura-50 p-6 rounded-lg shadow-inner mb-8">
        <h2 className="text-xl font-semibold text-aura-800 mb-3">
        ...
        {cart && cart.items.length > 0 ? (
          <>
            {cart.items.map((item) => (
              <p key={item._id} className="text-aura-700">
                ...
                {formatCurrency(
                  (item.selectedVariant?.price || item.productId?.basePrice) *
                    item.quantity,
                )}
              </p>
            ))}
            <div className="border-t border-aura-200 mt-4 pt-4 space-y-2">
              ...
              <div className="flex justify-between">
                <span>Shipping:</span>
                <span>{formatCurrency(100.0)}</span>
              </div>
              <div className="flex justify-between">
          ...
          <p className="text-aura-600">
            Your cart is empty. A dummy product will be added.
          </p>
        )}
      </div>
      ...
      <Link
        to="/products"
        className="btn-secondary mt-4 block w-full max-w-xs mx-auto"
      >
        Continue Shopping
      </Link>
    </div>
