...
function createCSVTemplate() {
  ...
  console.log('📝 Fill this file with your products and run: node bulk-import.js import');
}
      ...
      fs.createReadStream(filename)
        ...
        .on('data', (row) => {
          ...
          const product = {
            ...
            salePrice: row.salePrice ? parseInt(row.salePrice) : parseInt(row.basePrice),
            ...
            stock: parseInt(row.stock),
            isActive: true,
            seo: { keywords: row.tags ? row.tags.split(',').map(tag => tag.trim()) : [] },
            reviews: []
          };
...
async function exportToCSV() {
  try {
    ...
    console.log(`✅ Exported ${products.length} products to products-export.csv`);
  ...
  try {
    ...
    switch (command) {
      ...
      default:
        ...
        console.log('  node bulk-import.js import    - Import products from products.csv');
        console.log('  node bulk-import.js export    - Export current products to CSV');
        console.log('');
