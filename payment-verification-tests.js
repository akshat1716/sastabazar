...
async function testRazorpayConfiguration() {
  ...
  console.log(
    `   Webhook Secret: ${TEST_CONFIG.razorpayWebhookSecret ? "Configured" : "Not configured"}`,
  );
}
...
async function testPaymentOrderCreation() {
  ...
  try {
    ...
    console.log(
      `   Warning: Could not cancel test order: ${cancelError.message}`,
    );
  }
}
...
async function testPaymentVerificationLogic() {
  ...
  console.log(
    `   Signature Verification: ${isValid ? "Valid" : "Invalid (expected for test data)"}`,
  );
}
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      if (response.status === 404) {
        throw new Error(
          `Endpoint ${endpoint.method} ${endpoint.path} not found`,
        );
      }

      console.log(
        `   ${endpoint.method} ${endpoint.path}: Available (status ${response.status})`,
      );
    } catch (error) {
...
async function testEnvironmentVariablesValidation() {
  ...
  const optionalVars = ["RAZORPAY_WEBHOOK_SECRET"];
...
async function testDatabaseConnectionForOrders() {
  try {
    ...
    console.log(
      `   Connection pool size: ${response.data.connectionPool?.size || "Unknown"}`,
    );
  } catch (error) {
...
async function testRateLimitingForPaymentEndpoints() {
  ...
  if (successCount < 3) {
    throw new Error(
      `Rate limiting too strict: ${successCount}/3 requests succeeded`,
    );
  }
...
async function runPaymentVerificationTests() {
  ...
  await runTest(
    ...
    testEnvironmentVariablesValidation,
  );
  await runTest(
    "Database Connection for Orders",
    testDatabaseConnectionForOrders,
  );
  await runTest(
    "CORS Configuration for Payment",
    testCorsConfigurationForPayment,
  );
  await runTest(
    "Rate Limiting for Payment Endpoints",
    testRateLimitingForPaymentEndpoints,
  );
  ...
  if (testResults.failed === 0) {
    console.log(
      "🎉 All payment verification tests passed! Payment system is ready.",
    );
    process.exit(0);
  } else {
    console.log(
      "⚠️ Some payment tests failed. Please review and fix issues before processing payments.",
    );
    process.exit(1);
  }
}
