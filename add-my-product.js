...
async function main() {
  try {
    ...
    const shortDescription = await askQuestion('Short Description (for cards): ');
    
    console.log('\nValid categories: apparel, home-goods, tech-accessories, art-prints');
    const category = await askQuestion('Category: ');
    
    const brand = await askQuestion('Brand (default: sastabazar): ') || 'sastabazar';
    const basePrice = await askQuestion('Base Price (₹): ');
    const salePrice = await askQuestion('Sale Price (₹) - leave empty if no sale: ');
