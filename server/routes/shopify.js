  ...
  try {
    if (!shopifyClient) {
      return res.status(503).json({
        success: false,
        message:
          "Shopify integration not configured. Please add valid Shopify credentials to .env file.",
    ...
    console.log(
      `✅ Successfully fetched ${response.data.products.length} products from Shopify.`,
    );
    res.status(200).json({
      success: true,
...
router.get("/orders", async (req, res) => {
  try {
    ...
    console.log(
      `✅ Successfully fetched ${response.data.orders.length} orders from Shopify.`,
    );
    res.status(200).json({
      success: true,
...
router.get("/customers", async (req, res) => {
  try {
    ...
    console.log(
      `✅ Successfully fetched ${response.data.customers.length} customers from Shopify.`,
    );
    res.status(200).json({
      success: true,
