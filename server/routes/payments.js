...
if (config.payments.razorpay.keyId && config.payments.razorpay.keySecret) {
  ...
  logger.warn(
    "⚠️ Razorpay keys not found. Payment routes will be disabled or use dummy data",
  );
}
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      // Calculate totals
      const { subtotal, shipping, tax, total, orderItems } =
        calculateOrderTotals(cart.items);
...
router.post("/stripe/create-checkout-session", auth, async (req, res) => {
  try {
    ...
    // Calculate totals
    const { subtotal, shipping, tax, total, orderItems } = calculateOrderTotals(
      cart.items,
    );
