...
const validateConfig = () => {
  ...
  if (config.isProduction) {
    ...
    if (config.jwt.secret.includes('your-super-secret') || 
      ...
      errors.push('JWT secret must be changed from default and be at least 32 characters');
    }
    ...
    if (!config.payments.razorpay.enabled && !config.payments.stripe.enabled) {
      errors.push('At least one payment gateway must be configured for production');
    }
