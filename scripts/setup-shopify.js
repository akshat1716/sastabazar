...
async function setupShopifyIntegration() {
  ...
  const hasShopifyStore = await question('Do you already have a Shopify store? (y/n): ')
  ...
  if (hasShopifyStore.toLowerCase() !== 'y') {
    ...
    if (continueSetup.toLowerCase() !== 'y') {
      console.log('Please create your Shopify store first, then run this script again.')
      rl.close()
      return
    }
  }
  ...
  const shopName = await question('Enter your Shopify store name (without .myshopify.com): ')
