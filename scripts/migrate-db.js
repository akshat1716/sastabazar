...
async function runMigration() {
  try {
    ...
    await mongoose.connection.db.collection('products').createIndex({ isFeatured: 1 });
    await mongoose.connection.db.collection('products').createIndex({ isOnSale: 1 });
    await mongoose.connection.db.collection('products').createIndex({ basePrice: 1 });
    ...
    // Cart indexes
    await mongoose.connection.db.collection('carts').createIndex({ userId: 1 }, { unique: true });
    await mongoose.connection.db.collection('carts').createIndex({ updatedAt: -1 });
    
    // Review indexes
    await mongoose.connection.db.collection('reviews').createIndex({ productId: 1 });
    ...
    await mongoose.connection.db.collection('reviews').createIndex({ createdAt: -1 });
    
    // Wishlist indexes
    await mongoose.connection.db.collection('wishlists').createIndex({ userId: 1 }, { unique: true });
    await mongoose.connection.db.collection('wishlists').createIndex({ updatedAt: -1 });
    ...
    const requiredCollections = ['users', 'products', 'orders', 'carts', 'reviews', 'wishlists'];
