...
const productTemplate = {
  ...
  description: 'Detailed description of your product. Include features, benefits, and any important details customers should know.',
...
async function listProducts() {
  try {
    const products = await Product.find({}, 'name basePrice salePrice category isActive').lean();
    console.log('\n📦 Current Products:');
    console.log('==================');
    products.forEach((product, index) => {
      console.log(`${index + 1}. ${product.name}`);
      console.log(`   Price: ₹${product.basePrice} ${product.isOnSale ? `(Sale: ₹${product.salePrice})` : ''}`);
